...
방금도 말했듯이, 건물을 짓는것에 비유하자면 HTML은 뼈대고, CSS는 살이다. 한 번 더 사람에 비유해보면, HTML이 사람의 얼굴이라고 했을 때, CSS는 화장이라고 할 수 있을 것이다. 
...
### 2-3. CSS 선택자
CSS 선택자라는 단어가 나왔으니, 우리는 선택자가 무엇을 뜻하는 것인지부터 알아야 한다. 거창한 것은 아니고,, 무엇인가에 스타일을 주고자 할 때 대상을 지정해야 하는데, 이것을 선택자라고 한다. CSS로 나타낼 때 구문은 다음과 같다.
```
선택자 { 속성 : 속성값 ; }
